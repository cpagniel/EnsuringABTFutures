%% plot_overview_map_all_stock_ABT %%
...
[~,icon] = legend(m,{['Gulf: ' num2str(roundn(mean(META.length(strcmp(META.stock,ss{1}))),0)) ...
    ...
    ' CFL (n = ' num2str(sum(strcmp(META.stock,ss{2}))) ')'],...
    ['SS: ' num2str(roundn(mean(META.length(strcmp(META.stock,ss{3}))),0)) ...
    ' CFL (n = ' num2str(sum(strcmp(META.stock,ss{3}))) ')']},'FontSize',14,...
...
cd([fdir '/figures']);
exportgraphics(gcf,'overview_map_all_stock_ABT.png','Resolution',300);
