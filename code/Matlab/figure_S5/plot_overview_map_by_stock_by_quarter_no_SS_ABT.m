%% plot_overview_map_by_stock_by_quarter_no_SS_ABT %%
% Sub-function of ABT.m; plots SSM tracks of all tags colored by
% stock and quarter without SS.
%
...
for i = 1:4
    ...
    [~,icon] = legend(m,{['Gulf: ' num2str(roundn(mean(SSM.length(ismember(SSM.TOPPID,META.toppID(strcmp(META.stock,ss{1}))) & SSM.Quarter == i),'omitnan'),0)) ...
        ...
        ' CFL (n = ' num2str(length(unique(SSM.TOPPID(ismember(SSM.TOPPID,META.toppID(strcmp(META.stock,ss{2}))) & SSM.Quarter == i)))) ')']},'FontSize',14,...
        'Position',[0.7 0.245 0.13 0.13]);
    ...
    cd([fdir '/figures']);
    if i == 1
        exportgraphics(gcf,'overview_map_Winter_no_SS_ABT.png','Resolution',300);
    elseif i == 2
        exportgraphics(gcf,'overview_map_Spring_no_SS_ABT.png','Resolution',300);
    elseif i == 3
        exportgraphics(gcf,'overview_map_Summer_no_SS_ABT.png','Resolution',300);
    elseif i == 4
        exportgraphics(gcf,'overview_map_Fall_no_SS_ABT.png','Resolution',300);
    end
